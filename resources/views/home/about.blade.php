<section class="client_section mb-5">
    <div class="container">
       <div class="heading_container heading_center">
          <h2 class="mb-4">
             About Us
          </h2>
       </div>
       <div>
       </div>
       <div class="py-3" style="font-family:'Montserrat', sans-serif; !important; font-size:20px;">
            <p>
               <strong class="text-info"> DIY - “do it yourself”</strong> literally the name says it all. It is a New York based,
                full-service luxury decor company. You want to do it yourself but buying
                the materials is just way too much! So here we are providing party
                supplies for your events. It doesn’t matter how small or large your event
                is, or what type of event you are having.
            </p>
        </div>


    </div>
 </section>

